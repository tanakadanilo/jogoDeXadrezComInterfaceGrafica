...
public class Tabuleiro {
    ...
    public Tabuleiro(int linhas, int colunas) {
        if (linhas < 1 || colunas < 1) {
            throw new ExcecaoTabuleiro("Erro ao criar tabuleiro," + " é necessário que exista ao menos uma linha e uma " + "coluna");
        }
        this.linhas = linhas;
    ...
    public Peca getPeca(Posicao p) {
        return this.pecas[p.getLinha()][p.getColuna()];
    }

    public Peca getPeca(posicaoXadrez p) {
        return this.pecas[p.getLinha()][p.getColuna()];
    }
}
